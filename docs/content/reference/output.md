...
describes what users get. Pick a format with `-o`, or let nihreporter choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
nihreporter <command> -o table   # aligned columns for reading
nihreporter <command> -o jsonl   # one JSON object per line, for piping
nihreporter <command> -o json    # a single JSON array
nihreporter <command> -o csv     # spreadsheet friendly
nihreporter <command> -o tsv     # tab-separated
nihreporter <command> -o url     # just the URL column
nihreporter <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
nihreporter <command> --fields id,title,url
```
...
```bash
nihreporter <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
nihreporter <command>            # a table, because this is a terminal
nihreporter <command> | wc -l    # JSONL, because this is a pipe
```
